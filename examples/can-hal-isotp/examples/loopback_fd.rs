// ISO-TP CAN FD loopback test: send and receive on the same SocketCAN interface.
//
// Same structure as loopback.rs but uses IsoTpFdChannel with CAN FD frames
// (64-byte payloads). Requires an FD-capable SocketCAN interface.
//
// Software requirements:
//   - Linux only (SocketCAN)
//   - A CAN FD-capable interface (physical or vcan with FD support)
//
// Virtual interface setup (vcan supports FD by default):
//   sudo modprobe vcan
...
//
// Usage:
//   cargo run --example loopback_fd -p can-hal-isotp-examples -- [interface]
//
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("ISO-TP CAN FD loopback test on '{ifname}'");
    println!("-------------------------------------------");

    // Test payloads of increasing size.
    let payloads: Vec<Vec<u8>> = vec![
        // 7 bytes: fits in an FD SF with classic 1-byte PCI
        vec![0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07],
        // 40 bytes: fits in an FD SF with 2-byte PCI
        (0..40).map(|i| i as u8).collect(),
        // 62 bytes: maximum FD SF payload (Normal addressing)
        (0..62).map(|i| i as u8).collect(),
        // 200 bytes: requires FF + CFs
        (0..200).map(|i| (i & 0xFF) as u8).collect(),
        // 4000 bytes: large multi-frame
